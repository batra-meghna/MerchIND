...
app.get('/login-register',(req,res) =>{
    if(!req.session.isLoggedIn){res.render("login-register");}
    else{res.redirect('/');}
});
    ...
    .then(([rows1]) => {
    	...
    	.then(([rows2]) =>{
    		...
    		for (var i=0;i<rows2.length;i++){
    			products.push({id:rows2[i].id , name:rows2[i].name , desc:rows2[i].description , price:rows2[i].price , image_url:rows2[i].image_url});
    		}
    		res.render('home',{
            name:rows1[0].name,prods:products
    	});
    ...
    dbConnection.execute("SELECT name,price,image_url,id from items").then(([rows]) =>{
        ...
        if (rows.length) {
            for (var i=0;i<rows.length;i++){
                product.push({name:rows[i].name,price:rows[i].price,image_url:rows[i].image_url,id:rows[i].id})
            }
            res.render("market",{prods:product});
            ;}   
        else {
        	res.render("market",{prods:product});
        }
...
app.post("/filter",ifNotLoggedin,(req,res,next) =>{
	...
	dbConnection.query("SELECT item_id from item_tags join tags on tags.id=item_tags.tag_id where tags.tag_name in (?)",[req.body.tags]).then(([rows1])=>{
        ...
        if (rows2.length) {
            for (var i=0;i<rows2.length;i++){
                product.push({name:rows2[i].name , price:rows2[i].price , image_url:rows2[i].image_url , id:rows2[i].id})
            }
            console.log(product)
            res.render("market",{prods:product});
...
(req,res,next) => {
    ...
    const {f_name,l_name,phone,user_name,user_pass} = req.body;
     ...
     function(req, res){
        try{
            ...
            dbConnection.execute("INSERT into items (name,description,image_url,seller_id,price) VALUES (?,?,?,?,?)",[name,description,image_url,id,price]).then(result => {
                res.render("add_item",{msg:"Item has Been Added Successfully!!"})
                 if(tags){
                ...
                console.log("NANAI!");
                    ...
                    val=[];
                    for (var i =0;i<rows2.length;i++){
                        val.push({id1:item_id,id2:rows2[i].id});
                    }
                    console.log("Hoopa");
                    console.log(val);
            ...
            console.log("Fail");
            res.render("add_item",{msg:"Please Enter Data Correctly!!"});
...
app.post('/product_details',ifNotLoggedin,(req,res,next)=>{
		...
		dbConnection.execute("Select tags.tag_name as tags from item_tags join tags on tags.id=item_tags.tag_id where item_tags.item_id=?",[req.body.id]).then(([rows])=>{
			...
			item_data=[];
			item_data.push({seller_name:rows[0].seller_name,contact:rows[0].contact,item_name:rows[0].name,description:rows[0].description,url:rows[0].image_url,price:rows[0].price});
			//console.log(item_tags);
			//console.log(item_data);

			res.render("product_details",{prod:item_data[0],tags:item_tags});
